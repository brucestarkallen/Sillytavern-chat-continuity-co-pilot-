...
- **Editor** — standing craft notes injected each turn. It patrols the defect floor (scenes circling the protagonist, characters/props vanishing, dead ambient world, agency theft, same-voice NPCs, rushed resolutions, phrase tics) **and** holds the story to a masterpiece bar (dead scenes that don't turn, on-the-nose dialogue, wasted dramatic irony, unpaid setups, escalation-by-volume, frictionless success). Every pass opens with one **NORTH STAR** line — the single highest-leverage improvement — followed by the numbered standing notes. It can run on a reply cadence *and* (on by default) **automatically when an episode concludes**: the editor reviews the aired episode first, and in Auto director mode the next episode is then designed with the fresh notes in hand — a writers'-room review→plan loop.
